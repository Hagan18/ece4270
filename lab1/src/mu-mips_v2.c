...
void check_right(void){
	...
	//int jumpNum = 4;
	//uint32_t right = get_right();
	instruction = get_right(mem_read_32(CURRENT_STATE.PC));
	...
	if(flag==0){ //If flag is set = instruction is 'left'
		...
		//MULT 
			case 0x18:
				...
				binInstruction = binInstruction >> 5;
				rs = binInstruction & 0x1F;
				tempMult = prevInstruction = instruction;
	NEXT_STATE.PC = CURRENT_STATE.PC + 4;CURRENT_STATE.REGS[rt] * CURRENT_STATE.REGS[rs];
			...
			case 0xC:
				...
				break;	
                        default:
                            ...
                            check_left(instruction,binInstruction);
		}
	}
}

void check_left( uint32_t binInstruction){
	else if ((flag)){
		printf("inside elseif\n");
                instruction = get_left(mem_read_32(CURRENT_STATE.PC));
		long int immediate;
		switch(instruction){
			case 0x8: //ADDI
 ...
 *************************************************/
uint32_t get_right(uint32_t value){
